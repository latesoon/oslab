...
5. `pte_create` 在`page_remove_pte`之后建立新的页表项并将其赋值给`get_pte`找到的页表项的地址,直接根据物理页号进行偏移并对标志位进行设置完成。
...
#### 两段代码相似的原因
在 RISC-V 的 `sv32`、`sv39` 和 `sv48` 模式中，虚拟地址都会被划分为多个部分，分别用于索引页目录（Page Directory）、页表（Page Table）和页表项（Page Table Entry）。不同的是，虚拟地址的位数不同，因此页表的层级和每一层的大小会有所不同。例如，`sv32` 只有两个层级，而`sv39` 和 `sv48` 有三个和四个层级。
...
```
可以看到在上面的代码中，通过对页目录项和虚拟地址的访问，可以进行页表项的查找，并根据`PTE_V`判定其有效性，而在页表项不存在时，需要分配新的物理页面，并将其映射到相应的虚拟地址空间。上面的两个功能在一个函数中实现，我们认为这是一种不太好的设计，因此有必要将两个功能拆开，原因如下：
1.	在很多情况下，我们可能只需要查找页表项，而不需要修改页表。如果拆分后，查找操作可以单独调用，这样增强了代码的复用性，避免不必要的页面分配和内存消耗。
2.	每个功能单独实现，更加易于管理、测试和修改每一个具体的功能，便于维护的同时减少了代码的复杂程度。
3.	当我们只想查找某一个页表项是否存在也不想进行分配时，这样合并的设计仍会创建新的页表项，很可能带来内存的浪费。
